...
def train_ensemble_model(best_params_xgb):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(train_df)):
        ...
        # CatBoost
        X_train_cat = train_df.iloc[train_idx].drop(columns=[TARGET, 'PID'])
        y_train_cat = train_df.iloc[train_idx][TARGET]
        X_val_cat = train_df.iloc[val_idx].drop(columns=[TARGET, 'PID'])
        y_val_cat = train_df.iloc[val_idx][TARGET]
        X_test_cat = test_df.drop(columns=['PID'])
        ...
        cat_model.fit(
            ...
            cat_features=np.where(X_train_cat.dtypes == 'category')[0]
        )

        cat_oof[val_idx] = cat_model.predict(X_val_cat).astype(int).flatten()
        cat_test_preds += cat_model.predict(X_test_cat).astype(int).flatten() / N_FOLDS
    ...
    submission.to_csv("submission_ensemble_xgb_lgb_cat.csv", index=False)
    print("Submission saved as submission_ensemble_xgb_lgb_cat.csv")
    return submission
